...
import { User as UserIcon, ShieldCheck, Lock, Book, PenLine, History, PieChart, Sparkles } from 'lucide-react';
...
function AppContent() {
      ...
      if (user) {
        ...
        if (!userDoc.exists()) {
          const newProfile = {
            ...
            nickname: user.displayName || '', photoURL: user.photoURL || '',
            streak: 0, lastWroteAt: null,
  ...
  if (loading) return (
    <div className="h-screen w-full flex items-center justify-center bg-night-950">
      <Sparkles className="animate-spin text-copper w-12 h-12" />
    </div>
  );

  if (!user) return (
    <div className="h-screen w-full flex flex-col items-center justify-center p-6 text-center gap-8">
      <Book className="w-20 h-20 text-copper" />
      <div>
        <h1 className="text-5xl font-serif font-bold mb-2 text-stone-100">StoryDiary</h1>
        <p className="text-stone-500 italic font-serif">Your AI-powered narrative journal</p>
      </div>
      <button onClick={() => signInWithPopup(auth, new GoogleAuthProvider())}
        className="bg-stone-100 text-night-950 px-8 py-4 rounded-2xl font-bold flex items-center gap-3 hover:bg-white transition-all">
      ...
      <nav className="fixed bottom-0 md:left-0 md:top-0 md:w-24 w-full bg-night-900/80 backdrop-blur-xl border-t md:border-r border-white/10 z-50 flex md:flex-col items-center justify-around py-4">
        {tabs.map(({ id, icon: Icon }) => (
          <button key={id} onClick={() => setActiveTab(id as any)}
            className={cn("p-3 rounded-2xl transition-all", activeTab === id ? "bg-copper text-white" : "text-stone-500 hover:text-stone-300")}>
            <Icon size={24} />
          </button>
        ))}
        <button onClick={() => savedPin ? setIsLocked(true) : setIsSettingPin(true)}
          className="p-3 text-stone-500 hover:text-stone-300 transition-all">
          {savedPin ? <Lock size={24} /> : <ShieldCheck size={24} />}
        </button>
      </nav>

      <main className="max-w-4xl mx-auto p-8 pb-28 md:pb-8">
